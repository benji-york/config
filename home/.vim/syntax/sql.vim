...
syn keyword sqlKeyword	cache compress connect current cursor decimal declare default desc
...
syn keyword sqlKeyword	public record resource return returns row rowlabel rownum rows
syn keyword sqlKeyword	session share size smallint type using
syn keyword sqlKeyword	join cross inner outer left right limit

syn keyword sqlConditional when then else
...
syn keyword sqlType	dec decimal float int integer long mlslabel nchar text
syn keyword sqlType	nclob number numeric nvarchar2 precision raw rowid double
...
syn keyword sqlFunction	substring
...
hi def link sqlCast             Operator
"hi sqlPlaceholder  cterm=NONE ctermfg=105  guifg=#ff40ff
hi def link sqlPlaceholder      PreProc
